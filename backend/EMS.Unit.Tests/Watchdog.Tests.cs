...
public class WatchdogTest
{
	...
	{
        using (new DateTimeProviderContext(new DateTime(2023, 6, 1, 12, 0, 0, DateTimeKind.Utc)))
        {
    ...
    public async Task RestartWhenDelay()
    {
        using (new DateTimeProviderContext(new DateTime(2023, 6, 1, 12, 0, 0, DateTimeKind.Utc)))
        {
            ...
            watcher.Register(worker.Object, 30);
            using (new DateTimeProviderContext(new DateTime(2023, 6, 1, 12, 0, 40, DateTimeKind.Utc)))
            {
                await watcher.PerformCheck().ConfigureAwait(false);
            }
